...
export default function StockAdjAcrst() {
    ...
    const columnDefs = useMemo(() => [
        ...
        {
            field: 'hldNo', headerName: '보류 번호', width: 155,
            headerTooltip: '소진한 보류 건 — 값이 있으면 보류분 폐기이고, 같은 번호의 해제 실적(사유: 재고조정)이 재고 보류 화면에 남는다. 비어 있으면 가용분 조정이다',
            cellRenderer: (p) => (p.value
                ? <span className="text-xs"><span className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 mr-1.5">보류분</span>{p.value}</span>
    ...
    // 조회는 1페이지부터, 페이저는 그 페이지로 (보류/해제 실적 화면과 같은 형태)
    const fetchList = async (nextPage = 1) => {
        setPage(nextPage);
        setData(await invAdjApi.list(cond, { page: nextPage, size }));
    };

    // 마운트 조회는 fetchList를 부르지 않고 API를 직접 호출한다 — 이펙트가 컴포넌트 함수를
    // 의존성으로 잡게 되어 react-hooks 규칙에 걸린다
    useEffect(() => {
        invAdjApi.list(initCond(), { page: 1, size }).then(setData);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // 합계는 이 페이지분이다 — 전체 합계를 내려면 셈 쿼리를 하나 더 둬야 하는데,
    // 조정은 「무엇을 얼마나 처분했나」를 건별로 읽는 화면이라 그만한 값이 없다
    const downQty = data.rows.reduce((s, r) => s + Math.min(r.adjQty, 0), 0);
    const upQty = data.rows.reduce((s, r) => s + Math.max(r.adjQty, 0), 0);
        ...
        <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex-1 min-h-0 flex flex-col gap-3">
                <span className="text-xs font-bold">
                    <span className="text-slate-500 font-medium mr-2">이 페이지 증감</span>
                ...
                <Pager page={page} size={size} totCnt={data.totCnt} label="조정 실적" onChange={(p) => fetchList(p)} />
            </div>
        </div>
    );
}
