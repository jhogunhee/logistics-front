...
/**
 * 재고 로트변경 실행. 행 단위가 Lot이 아니라 **재고 행(로케이션 포함)**이고 수량을 지정한다 —
 * 재고 속성변경(Lot 전량 일괄)과 갈리는 지점. 목록은 보관 로케이션 + 유통기한 관리 상품 +
 * 가용수량 > 0 으로 서버가 거른다.
 *
 * 목적지 배치는 셀에 직접 타이핑하지 않고 **모달에서 고른다** — 같은 상품+입고일자의 기존 Lot을
 * 고르면 그 날짜가 그대로 실려 병합이 되고(유통기한 불일치 거부를 입력 단계에서 차단),
 * 후보가 없으면 직접 입력이 분할(새 Lot 채번)이 된다. 후보 규칙은 서버가 강제한다.
 */
