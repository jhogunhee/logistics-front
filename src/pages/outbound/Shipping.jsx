...
export default function Shipping() {
    ...
    return (
        <div className="flex flex-col gap-4 h-full min-h-[36rem]">
            ...
            {/* 상: 발행된 웨이브(단일 선택) / 하: 주문 그리드(체크 = 확정 대상) */}
            <PanelGroup direction="vertical" autoSaveId="outb-shipping-split-v1" className="flex-1 min-h-0">
                ...
                <Panel defaultSize={60} minSize={25} className="flex flex-col gap-2 min-h-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-400 truncate">
                            {wave ? `${wave.wavNo} · 확정대상 ${readyRows.length}건` : '위에서 웨이브를 선택하세요'}
                        </span>
