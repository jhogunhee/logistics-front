...
export const outbOrderApi = {
    ...
    // 등록은 없다 — 출고주문은 OMS 주문확정(omsOutbOrderApi.confirm)으로만 생성된다.
    // 서버에도 POST /outbound/orders 가 없으므로 여기에 되살리지 말 것.

    /** 주문 취소 (할당 전 CREATED만). 상위 주문의 확정취소와는 다르다 —
     *  이쪽은 창고 문서만 CANCELLED로 눕히고 OMS 주문은 확정으로 남는다 */
    cancel(outbOrderId) {
        return api.post(`/outbound/orders/${outbOrderId}/cancel`);
    },
};
...
export const OUTB_STATUS_META = {
    ...
    CANCELLED: { label: '취소',     badge: 'bg-slate-100 text-slate-400' },
};
